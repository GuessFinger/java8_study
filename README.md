...
#### 2019年5月5日21:21:11
- 因为这一章的内容和之前的内容关联性比较大 所以这一章的内容依旧放到day13中进行学习

![TemporalAdjuster类的工厂方法](./images/TemporalAdjuster类的工厂方法)
---
![日期处理补充说明](./images/日期处理补充说明.png)
---
![时区说明1](./images/时区说明1.png)

#### 2019年5月6日20:33:02
- day14

- 我们在对现有的系统进行修改的时候 有一点需要考虑就是 对于修改这个的点 其他的模块是不是和这个有关联 
    可预见范围内 尽可能的处理好  我们在使用变量的时候 有一个概念就是无副作用 就是这个对象一完成初始化
    就不会被任何方法修改 它不会进入一个不可预期的状态

- 一般来说我们实现一个系统 有两种思考 一种是专注于如何实现 比如是先实现这个 然后进行下一步的处理
    这个怎么说呢？逻辑性更强 更着重于实现 我们最终的目的就是实现这个东西 书中称之为这个为命令式编程
    
    另一种就是关注我们要做什么 我们把其中的更细节的东西交给函数库去处理 我们重点是这个东西要干什么
    别人在浏览的时候是否能够更加清楚的看明白这个东西
---
![函数式编程说明](./images/函数式编程说明.png)
---

- 函数式编程 像数学函数一样 传进入一个值 通过一定的计算 得到一个值  不管你执行多少次 只要传入的值一样
    那么得到的值也一定是一样的
![函数式编程说明1](./images/函数式编程说明1.png)
---
![函数式编程说明2](./images/函数式编程说明2.png)
---
![函数式编程说明3](./images/函数式编程说明3.png)
---
![引用透明性](./images/引用透明性.png)

- 刚才看到函数式编程的例子 印象最深的额一点就是 你内部的处理 尽量是不会传入的参数进行任何的修改
    哪怕是重新创建一个新的对象  都不在原来的基础上进行任何的修改 我想这么做的目的就是为了 
    为了防止其中出现任何不可理预见的错误 因为你使用的是创建对象的副本
    
- 刚才看得例子就是 之前再阿里巴巴规范中说明的问题 你在循环中不能更改对象的值 否则可能造成死循环
    最直观的一个例子就是 在循环的过程中 改变数组的长度 用foreach循环 
    但是如果你用iterator的话 就不会产生这个问题 
    这个就是我们所说的副作用
    
#### 2019年5月7日20:28:30
- day15
- 函数式编程:指函数或者方法的行为应该像 数学函数一样 没有任何的副作用。能够像普通变量一样使用的函数
    我们称之为一等函数。
    
- 函数式编程世界里面 如果函数 接受至少一个函数作为参数 返回的结果是另一个参数  我们就成为之高阶函数

![科里化](./images/科里化.png)


- 后面说的问题 和之前整理的内容比较相似 就是我们在进行参数的传递的时候 尽量进行参数的副本化  不改变
    参数的原来状态 为了就是防止这个产生一些不可预期的问题
    
    
- 我发现在自己现在再写后台代码的时候 根本没有一个统一的思想 或者说没有一个整体的思路 来规划
    最直观的一点 就是你在对对象操作的时候  使用传统的java操作方式的话 书中说你最好进行一个复制操作
    有一种情况就是 你把对象a传递进去以后 对对象a进行了修改 但是其他的地方同样用到对象a 那么这个就可能发生一些
        不可预料的问题  这个也可以说就是一个副作用  我们为什么要采用函数式编程呢？ 
        没有副作用 传入什么参数 你执行若干次 还是同样的结果